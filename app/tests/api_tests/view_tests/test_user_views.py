...
class UserViewTests(ViewTestCase):
    ...
    def test_patch_user(self, app, client):
        """Test 200 response returned when patching user."""

        test_user = create_user()
        self.interface.add_to_db(test_user)
        ...
        res = client.patch(f"/api/user-item/{test_user.id}",
        ...
        self.assertEqual(updated_user["name"], patch_data["name"])
        self.assertEqual(updated_user["username"], patch_data["username"])
        self.assertEqual(updated_user["email"], patch_data["email"])
        self.assertEqual(updated_user["phone"], "1234567890")

    def test_delete_user(self, app, client):
        """Test 204 response when deleting user. Assure user is indeed deleted."""
        ...
        res = client.delete(f"/api/user-item/{test_user.id}")
        self.assertEqual(res.status_code, 204)

        deleted_user = self.interface.get_object(User, id=test_user.id)
